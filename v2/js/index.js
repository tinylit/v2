function initPage() {
    ...
    var z = "`${for(var item<index> in x?.cc[0]){ return $'第{index}个是{item};'; }}`".withCb({ x: { cc: { 0: [1, 2, 3] } } });
    ...
    var str = "${displayName??name}是一个`${description}`的、基于{{mode}}开发的前端框架。".withCb({
        displayName: null, name: "v2", description: "轻量高效", mode: "流程"
    });
}